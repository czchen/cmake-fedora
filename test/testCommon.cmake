SET(CMAKE_ALLOW_LOOSE_LOOP_CONSTRUCTS ON)
SET(CTEST_SOURCE_DIRECTORY "${CTEST_SCRIPT_DIRECTORY}/..")
SET(CTEST_BINARY_DIRECTORY "${CTEST_SOURCE_DIRECTORY}")
#SET(CTEST_COMMAND "${CTEST_EXECUTABLE_NAME}")
IF(NOT DEFINED CTEST_COMMAND)
    SET(CTEST_COMMAND "echo 'Done.'")
ENDIF(NOT DEFINED CTEST_COMMAND)

SET(CMAKE_MODULE_PATH ${CMAKE_MODULE_PATH} src test ${CTEST_SOURCE_DIRECTORY}/src ${CTEST_SOURCE_DIRECTORY}/test)
MESSAGE("CMAKE_MODULE_PATH=${CMAKE_MODULE_PATH}")

MACRO(TEST_STR_MATCH var value)
    SET(_val "${${var}}")
    MESSAGE("var=${var} _val=|${_val}| value=|${value}|")
    IF(NOT "${_val}" STREQUAL "${value}")
	MESSAGE(SEND_ERROR "  Error: ${var}=|${_val}|${value}|")
    ENDIF()
ENDMACRO()

#EXECUTE_PROCESS(COMMAND pwd
#    OUTPUT_VARIABLE PWD
#    OUTPUT_STRIP_TRAILING_WHITESPACE)
#SET(CTEST_SOURCE_DIRECTORY  "${PWD}")
#SET(CTEST_BINARY_DIRECTORY  "${CTEST_SOURCE_DIRECTORY}")

