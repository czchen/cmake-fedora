...
FUNCTION(ZANATA_PARSE_LOCALE language script country str)
    SET(l "")
    SET(s "")
    SET(c "")
    IF("${str}" MATCHES "([a-z]*)[_-](.*)")
	SET(l "${CMAKE_MATCH_1}")
	SET(x "${CMAKE_MATCH_2}")
	IF("${x}" MATCHES "[A-Z]*")
	    SET(c "${x}")
...
FUNCTION(ZANATA_ZANATA_XML_MAP zanataXml zanataXmlIn clientLocales)
	...
	IF("${line}" MATCHES "<locale>(.*)</locale>")
	    ...
	    ## Find the best match
	    SET(bestMatch "")
	    ZANATA_PARSE_LOCALE(sLang sScript sCountry "${sL}")
		...
		ZANATA_PARSE_LOCALE(cLang cScript cCountry "${cL}") 
